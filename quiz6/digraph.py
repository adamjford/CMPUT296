...
class Digraph:
    ...
    def is_path(self, path):
        ...
        for i in range(len(path)-1):
            if path[i+1] not in self._tosets[path[i]]: return False
        return True
...
def shortest_path(G, source, dest):
    ...
    True
    """
    return least_cost_path(G, source, dest, lambda x: 1)
...
def least_cost_path(G, start, dest, cost):
    ...
    while todo and (dest not in visited):
        ...
        # get smallest estimated cost vertex from todo list
