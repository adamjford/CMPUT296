...
def do_step():
    ...
    # time complexity of this is O( z n log(n) )

    for z in Zombie.get_all_instances():
        if z.is_present():

            targets = sorted(
            # make pairs of (person, edge_edge distance from self to person)
            [ (p, z.distances_to(p)[3] ) for p in
                Normal.get_all_instances() if p.is_present() ]
                ,
            ...
            for (p, d) in targets:
                if d <= p.get_touching_threshold():
                    x = p.get_xpos()
                    y = p.get_ypos()

                    new_z = Zombie(size=person_size, haircolor='green', 
